import { getPriceData, getAddressDistribution100to1kData } from '@/lib/sheets'
import AddressDistributionChart from '@/components/charts/AddressDistributionChart'

export default async function KAS100to1kDistributionPage() {
    ...
    <div className="min-h-screen bg-[#0A0A12] text-white">
      <div className="p-6">
          ...
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-bold mb-2 bg-gradient-to-b from-[#FFFFFF] to-[#A0A0B8] bg-clip-text text-transparent drop-shadow-sm">
                Address Distribution: 100-1K KAS
              </h1>
              <p className="text-[#6B7280] text-lg">
                Analysis of addresses holding between 100 and 1,000 KAS tokens over time
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 text-sm text-[#6B7280] mb-6">
          ...
          <span className="text-[#5B6CFF]">100-1K KAS</span>
        </div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white mt-1">
                  {addressData.length > 0 
                    ? (addressData[addressData.length - 1].value >= 1000 
                        ? (addressData[addressData.length - 1].value / 1000).toFixed(1) + 'K'
                        : addressData[addressData.length - 1].value.toFixed(0))
                    : '--'
                  }
                </p>
                <p className="text-[#10B981] text-xs mt-1">
                  {addressData.length > 1 
                    ? ((addressData[addressData.length - 1].value - addressData[addressData.length - 2].value) >= 0 ? '+' : '') 
                      + (Math.abs(addressData[addressData.length - 1].value - addressData[addressData.length - 2].value) >= 1000
                        ? ((addressData[addressData.length - 1].value - addressData[addressData.length - 2].value) / 1000).toFixed(1) + 'K'
                        : (addressData[addressData.length - 1].value - addressData[addressData.length - 2].value).toFixed(0))
                    : 'Live Data'
                  }
                </p>
              </div>
          ...
          <div className="bg-[#1A1A2E]/50 backdrop-blur-sm border border-[#2D2D45]/30 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white mt-1">
                  {addressData.length > 0 
                    ? (Math.max(...addressData.map(d => d.value)) >= 1000 
                        ? (Math.max(...addressData.map(d => d.value)) / 1000).toFixed(1) + 'K'
                        : Math.max(...addressData.map(d => d.value)).toFixed(0))
                    : '--'
                  }
                </p>
          ...
          <div className="bg-[#1A1A2E]/50 backdrop-blur-sm border border-[#2D2D45]/30 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-[#2D2D45]/20">
                ...
                <span className="text-white text-sm font-medium">100 - 1,000 KAS</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-[#2D2D45]/20">
                <span className="text-[#6B7280] text-sm">Category</span>
                <span className="text-white text-sm font-medium">Small Retail Investors</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-[#2D2D45]/20">
                <span className="text-[#6B7280] text-sm">Typical Profile</span>
                <span className="text-white text-sm font-medium">Active Participants</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-[#6B7280] text-sm">Market Segment</span>
                <span className="text-white text-sm font-medium">Growing Interest</span>
              </div>
            </div>
          </div>
              ...
              <div className="p-3 bg-[#0F0F1A]/40 rounded-lg">
                <div className="flex items-start space-x-2">
                  ...
                  <p className="text-[#A0A0B8] text-sm">
                    This tier represents committed retail investors who have moved beyond minimal holdings, showing stronger engagement with the Kaspa ecosystem.
                  </p>
                </div>
              </div>
              <div className="p-3 bg-[#0F0F1A]/40 rounded-lg">
                <div className="flex items-start space-x-2">
                  ...
                  <p className="text-[#A0A0B8] text-sm">
                    Growth in this segment often indicates increasing confidence and long-term investment interest in the network.
                  </p>
                </div>
              </div>
              <div className="p-3 bg-[#0F0F1A]/40 rounded-lg">
                <div className="flex items-start space-x-2">
                  ...
                  <p className="text-[#A0A0B8] text-sm">
                    This tier typically shows more stability than the 1-100 KAS segment, with less frequent but more meaningful position changes.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8 bg-gradient-to-r from-[#5B6CFF]/10 to-[#6366F1]/10 border border-[#5B6CFF]/20 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-[#A0A0B8]">
            <div>
              <p className="mb-2">
                <strong className="text-white">Data Source:</strong> Daily snapshots from Kaspa network analysis, tracking addresses with holdings between 100-1,000 KAS.
              </p>
              <p>
