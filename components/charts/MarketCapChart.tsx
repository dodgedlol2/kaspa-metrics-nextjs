...
// Enhanced currency formatting for market cap
...
export default function MarketCapChart({ data, height = 600 }: MarketCapChartProps) {
  const [marketCapScale, setMarketCapScale] = useState<'Linear' | 'Log'>('Log')
  ...
  const plotlyData = useMemo(() => {
    ...
    // Main market cap trace using regular scatter for gradient support
    traces.push({
      x: xValues,
      y: yValues,
      mode: 'lines',
      type: 'scatter', // Changed from scattergl to scatter for gradient support
      name: 'Kaspa Market Cap',
      line: { 
        color: '#5B6CFF', 
        width: 2 
      },
      fill: marketCapScale === 'Log' ? 'tonexty' : 'tozeroy',
      fillgradient: {
        type: "vertical",
        colorscale: [
      ...
      hovertemplate: timeScale === 'Linear' 
        ? '<b>%{fullData.name}</b><br>Market Cap: %{y}<extra></extra>'
        : '%{text}<br><b>%{fullData.name}</b><br>Market Cap: %{y}<extra></extra>',
      hoverinfo: 'none',
    ...
    if (athData) {
      ...
      traces.push({
        ...
        text: [`High ${formatMarketCap(athData.marketCap)}`],
        textposition: 'top left',
        textfont: { color: '#ffffff', size: 11 },
        showlegend: true,
        hovertemplate: `<b>High</b><br>Market Cap: ${formatMarketCap(athData.marketCap)}<br>Date: ${athData.date.toLocaleDateString()}<extra></extra>`,
      })
    }
    ...
    if (oylData) {
      ...
      traces.push({
        ...
        text: [`Low ${formatMarketCap(oylData.marketCap)}`],
        textposition: 'bottom left',
        textfont: { color: '#ffffff', size: 11 },
        showlegend: false,
        hovertemplate: `<b>Low</b><br>Market Cap: ${formatMarketCap(oylData.marketCap)}<br>Date: ${oylData.date.toLocaleDateString()}<extra></extra>`,
      })
    }

    return traces
  }, [filteredData, timeScale, marketCapScale, powerLawData, athData, oylData])
  ...
  const plotlyLayout = useMemo(() => {
    ...
    if (marketCapScale === 'Log') {
      ...
      yTickText = yTickVals.map(val => formatMarketCap(val))
    ...
    layout.yaxis = {
      ...
      type: marketCapScale === 'Log' ? 'log' : 'linear',
      gridcolor: '#363650',
      gridwidth: 1,
      color: '#9CA3AF',
      range: marketCapScale === 'Log' 
    ...
    if (marketCapScale === 'Log' && yTickVals && yTickText) {
    ...
    return layout
  }, [filteredData, timeScale, marketCapScale, athData, height])
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative group">
            <button className="flex items-center space-x-1.5 bg-[#1A1A2E] rounded-md px-2.5 py-1.5 text-xs text-white hover:bg-[#2A2A3E] transition-all duration-200">
              ...
              <span className="text-[#A0A0B8] text-xs">Market Cap Scale:</span>
              <span className="font-medium text-[#FFFFFF] text-xs">{marketCapScale}</span>
              ...
              <div className="p-1.5">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className={`font-medium text-xs ${marketCapScale === 'Linear' ? 'text-[#5B6CFF]' : 'text-[#FFFFFF]'}`}>
                      Linear Scale
                    </div>
                    <div className="text-[10px] text-[#9CA3AF] mt-0.5">
                      Equal spacing between market cap intervals
                    </div>
                  </div>
                </div>
                <div 
                  onClick={() => setMarketCapScale('Log')}
                  className={`flex items-center space-x-2.5 p-2.5 rounded-md cursor-pointer transition-all duration-150 ${
                    marketCapScale === 'Log' 
                      ? 'bg-[#5B6CFF]/20' 
                      : 'hover:bg-[#1A1A2E]/80'
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className={`font-medium text-xs ${marketCapScale === 'Log' ? 'text-[#5B6CFF]' : 'text-[#FFFFFF]'}`}>
                      Logarithmic Scale
                    </div>
                    <div className="text-[10px] text-[#9CA3AF] mt-0.5">
              ...
              <div className="p-1.5">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[10px] text-[#9CA3AF] mt-0.5">
                      Display only the market cap data
                    </div>
                  </div>
                </div>
                <div 
