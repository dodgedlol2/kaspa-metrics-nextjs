...
interface PriceHashrateVolume3DChartProps {
  priceData: DataPoint[]
...
function fit3DPowerLaw(data: Array<{hashrate: number, price: number, volume: number}>) {
  ...
  try {
    ...
    let ssRes = 0, ssTot = 0
    for (const d of logData) {
      const predicted = Math.log(A) + B * d.lnHashrate + C * d.lnVolume
  ...
  const chartData = useMemo(() => {
    ...
    switch (colorBy) {
      ...
      case 'Hashrate':
        ...
        break
      case 'Volume':
        colors = filteredAnalysisData.map(d => d.volume)
        colorscale = 'Cividis'
        colorbarTitle = 'Volume (USD)'
        break
    }

    // 3D Scatter plot with hashrate, price, volume
    allTraces.push({
      x: filteredAnalysisData.map(d => d.hashrate),
      y: filteredAnalysisData.map(d => d.price),
      z: filteredAnalysisData.map(d => d.volume),
      mode: 'markers',
      type: 'scatter3d',
      name: 'Hashrate vs Price vs Volume',
      marker: {
        size: 4,
        color: colors,
    ...
    if (showPowerLaw === 'Show' && powerLaw3D) {
      ...
      // In log-log-log space, this should appear perfectly straight
      ...
      for (let i = 0; i <= numPoints; i++) {
        ...
        const logVolume = logVolumeMin + (logVolumeMax - logVolumeMin) * t
      ...
      <div className={`bg-[#1A1A2E] rounded-xl p-6 ${className}`}>
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <div className="text-[#6B7280] text-sm">Unable to correlate price, hashrate, and volume data for 3D visualization</div>
          </div>
        </div>
      </div>
    )
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative group">
            ...
            <div className="absolute top-full mt-1 left-0 w-56 bg-[#0F0F1A]/60 border border-[#2D2D45]/50 rounded-lg shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-20 backdrop-blur-md">
              <div className="p-1.5">
                {(['Time', 'Price', 'Hashrate', 'Volume'] as const).map((option) => (
                  <div 
                    key={option}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-[10px] text-[#9CA3AF] mt-0.5">
                        ...
                        {option === 'Volume' && 'Color intensity reflects trading activity'}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="p-1.5">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[10px] text-[#9CA3AF] mt-0.5">
                      Connect points to show market evolution path
                    </div>
                  </div>
                </div>
              </div>
        ...
        <Plot
          ...
          config={{
            ...
            toImageButtonOptions: {
              format: 'png',
              filename: 'price_hashrate_volume_3d_chart',
              height: 800,
              width: 1200,
              scale: 1
            }
