...
// Dynamically import Plotly to avoid SSR issuess
...
function formatAddressCount(value: number, forceUnit?: string): string {
  if (forceUnit === 'K' || (!forceUnit && value >= 1000)) {
...
// Generate log ticks for address counts
...
export default function AddressDistributionChart({ data, priceData, height = 600, tierName, tierRange }: AddressDistributionChartProps) {
  ...
  const plotlyData = useMemo(() => {
    ...
    // Main address count trace using regular scatter for gradient support
    traces.push({
      x: xValues,
      y: yValues,
      mode: 'lines',
      type: 'scatter', // Changed from scattergl to scatter for gradient support
      name: `${tierName} Addresses`,
      line: { 
        color: '#5B6CFF', 
        width: 2 
      },
      fill: addressScale === 'Log' ? 'tonexty' : 'tozeroy',
      fillgradient: {
        type: "vertical",
        colorscale: [
      ...
      connectgaps: true,
      hovertemplate: timeScale === 'Linear' 
        ? '<b>%{fullData.name}</b><br>Addresses: %{y}<extra></extra>'
        : '%{text}<br><b>%{fullData.name}</b><br>Addresses: %{y}<extra></extra>',
      hoverinfo: 'none',
  ...
  const plotlyLayout = useMemo(() => {
    ...
    if (addressScale === 'Log') {
      ...
      yTickText = yTickVals.map(val => formatAddressCount(val))
      ...
      yTickText = linearTicks.map(val => formatAddressCount(val))
    }
    ...
    // Configure primary Y-axis (addresses)
    layout.yaxis = {
      title: { text: 'Number of Addresses' },
      type: addressScale === 'Log' ? 'log' : 'linear',
      gridcolor: '#363650',
      gridwidth: 1,
      color: '#9CA3AF',
      range: addressScale === 'Log' 
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative group">
            <button className="flex items-center space-x-1.5 bg-[#1A1A2E] rounded-md px-2.5 py-1.5 text-xs text-white hover:bg-[#2A2A3E] transition-all duration-200">
              ...
              <span className="text-[#A0A0B8] text-xs">Address Scale:</span>
              <span className="font-medium text-[#FFFFFF] text-xs">{addressScale}</span>
            ...
            <div className="absolute top-full mt-1 left-0 w-64 bg-[#0F0F1A]/60 border border-[#2D2D45]/50 rounded-lg shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-20 backdrop-blur-md">
              <div className="p-1.5">
                <div 
                  onClick={() => setAddressScale('Linear')}
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className={`font-medium text-xs ${addressScale === 'Linear' ? 'text-[#5B6CFF]' : 'text-[#FFFFFF]'}`}>
                      Linear Scale
                    </div>
                    <div className="text-[10px] text-[#9CA3AF] mt-0.5">
                      Equal spacing between address counts
                    </div>
                  </div>
                </div>
                <div 
                  onClick={() => setAddressScale('Log')}
                  className={`flex items-center space-x-2.5 p-2.5 rounded-md cursor-pointer transition-all duration-150 ${
                    addressScale === 'Log' 
                      ? 'bg-[#5B6CFF]/20' 
                      : 'hover:bg-[#1A1A2E]/80'
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className={`font-medium text-xs ${addressScale === 'Log' ? 'text-[#5B6CFF]' : 'text-[#FFFFFF]'}`}>
                      Logarithmic Scale
                    </div>
                    <div className="text-[10px] text-[#9CA3AF] mt-0.5">
